...
func (k Keeper) SetBlockHeaders(ctx sdk.Context, blockHeaders []*types.BlockHeader) error {
	...
	for _, header := range blockHeaders {
		...
		if best.Hash != header.PreviousBlockHash {
			...
			// check if the new block header has more work than the work cumulated from the old block to the current tip
			totalWorkOldToTip := k.CalcTotalWork(ctx, header.Height, best.Height)
			workNew := blockchain.CalcWork(types.BitsToTargetUint32(header.Bits))
