...
func (m msgServer) Refresh(goCtx context.Context, msg *types.MsgRefresh) (*types.MsgRefreshResponse, error) {
	...
	for _, dkgId := range msg.DkgIds {
		...
		if remainingParticipantNum < tsstypes.MinDKGParticipantNum {
			return nil, errorsmod.Wrapf(types.ErrInvalidParticipants, "remaining participants %d cannot be less than min participants %d for dkg %d", remainingParticipantNum, tsstypes.MinDKGParticipantNum, dkgId)
		}
